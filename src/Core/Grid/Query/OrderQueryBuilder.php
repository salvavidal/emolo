...
final class OrderQueryBuilder implements DoctrineQueryBuilderInterface
{
    ...
    public function getSearchQueryBuilder(SearchCriteriaInterface $searchCriteria)
    {
		...
		// Obtener el ID del empleado y su perfil
    $id_employee = $this->getEmployeeIdFromSession();
    $employee = new \Employee($id_employee);
    $id_profile = $employee->id_profile;

    if ($id_profile != _PS_ADMIN_PROFILE_) {
        // Filtrar pedidos por los clientes asignados al empleado si no es administrador
        $qb->andWhere('o.id_customer IN (
