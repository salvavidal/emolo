...
class Fs2psUploadPdfsModelosTask extends Fs2psTask
{
    public function __construct($mng, $cmd)
    {
        parent::__construct('upload_pdfs_modelos', $mng, $cmd);
        $this->folder = dirname(__FILE__).'/../fwpdfsmodelos/pdfs/';
        if (!file_exists($this->folder)) {
            mkdir($this->folder, 0770, true);
            file_put_contents($this->folder.'index.html', '');
        }
    }
    
    protected function _execute($cmd)
    {
        if (empty($cmd['pdfs_modelos'])) return;
        foreach ($cmd['pdfs_modelos'] as $file)
        {
            file_put_contents($this->folder.$file['name'], base64_decode($file['data']));
        }
    }
